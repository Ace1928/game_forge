    ...
    def reproduce(self) -> None:
        ...
        for ct in self.cellular_types:
            ...
            def mutate_trait(
                ...
                if np.any(mutation_mask):
                    mutation = np.random.uniform(
                        min_range, max_range, size=mutation_mask.sum()
                    )
            ...
            if ct.mass_based and ct.mass is not None:
                ...
                mass_mutation_mask = np.random.random(num_offspring) < 0.1

                if np.any(mass_mutation_mask):
                    offspring_mass[mass_mutation_mask] *= np.random.uniform(
                        0.95, 1.05, size=mass_mutation_mask.sum()
                    )
            ...
            max_species_id = (
                np.max(ct.species_id) if ct.species_id.size > 0 else ct.type_id
            )
            species_ids = np.where(
            ...
            for i in range(num_offspring):
                try:
                    ...
                    ct.add_component(
                        x=ct.x[parent_indices[i]] + np.random.uniform(-5, 5),
                        y=ct.y[parent_indices[i]] + np.random.uniform(-5, 5),
                        ...
                        energy_efficiency_val=offspring_traits["energy_efficiency"][i],
