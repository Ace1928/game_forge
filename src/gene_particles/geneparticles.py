...
class GeneticParamConfig:
    ...
    def clamp_gene_values(
        ...
        speed_factor = np.clip(
            speed_factor, self.speed_factor_range[0], self.speed_factor_range[1]
        )
        interaction_strength = np.clip(
            interaction_strength,
            self.interaction_strength_range[0],
            self.interaction_strength_range[1],
        )
        perception_range = np.clip(
            perception_range,
            self.perception_range_range[0],
            self.perception_range_range[1],
        )
        reproduction_rate = np.clip(
            reproduction_rate,
            self.reproduction_rate_range[0],
            self.reproduction_rate_range[1],
        )
        synergy_affinity = np.clip(
            synergy_affinity,
            self.synergy_affinity_range[0],
            self.synergy_affinity_range[1],
        )
        colony_factor = np.clip(
            colony_factor, self.colony_factor_range[0], self.colony_factor_range[1]
        )
        drift_sensitivity = np.clip(
            drift_sensitivity,
            self.drift_sensitivity_range[0],
            self.drift_sensitivity_range[1],
        )
        return (
            speed_factor,
            interaction_strength,
...
class SimulationConfig:
    ...
    def __init__(self):
        ...
        self.interaction_strength_range: Tuple[float, float] = (
            -3.0,
            3.0,
        )  # Wider interaction range
    ...
    def _validate(self) -> None:
        ...
        if self.interaction_strength_range[0] >= self.interaction_strength_range[1]:
            raise ValueError(
                "Interaction strength range must have the first value less than the second."
            )
        ...
        if self.genetics.gene_mutation_range[0] >= self.genetics.gene_mutation_range[1]:
            raise ValueError(
                "Gene mutation range must have the first value less than the second."
            )

        if self.energy_efficiency_range[0] >= self.energy_efficiency_range[1]:
            raise ValueError(
                "Energy efficiency range must have the first value less than the second."
            )
        ...
        if (
            ...
            raise ValueError(
                "Energy efficiency mutation range must have the first value less than the second."
            )
...
class InteractionRules:
    ...
    def __init__(self, config: SimulationConfig, mass_based_type_indices: List[int]):
        ...
        self.mass_based_type_indices = (
            mass_based_type_indices  # Indices of mass-based cellular types
        )
    ...
    def _random_interaction_params(self, i: int, j: int) -> Dict[str, Any]:
        ...
        if random.random() < 0.5:
            potential_strength = (
                -potential_strength
            )  # Randomly invert potential strength
    ...
    def _create_synergy_matrix(self) -> np.ndarray:
        """
        Create a synergy matrix defining energy sharing relationships between cellular types.

        The matrix encodes the strength of synergistic alliances where higher values
        indicate stronger energy-sharing relationships. Each element (i,j) determines
        how much energy is shared from type i to type j when they interact within
        the synergy range.

        Returns
        -------
        np.ndarray
            NxN float matrix where element (i,j) represents the synergy factor
            between cellular type i and j (range: 0.0 to 0.9). Zero values
            indicate no synergistic relationship exists.
        """
        # Initialize matrix with zeros (no synergy by default)
        synergy_matrix = np.zeros(
            (self.config.n_cell_types, self.config.n_cell_types), dtype=float
        )

        # Establish synergy relationships between non-identical cellular types
        for i in range(self.config.n_cell_types):
            for j in range(self.config.n_cell_types):
                # Skip self-synergy relationships (diagonal elements)
                if i != j:
                    # 10% chance of establishing a synergy relationship
                    if random.random() < 0.1:
                        # Assign random synergy factor between 0.1 and 0.9
                        synergy_matrix[i, j] = random.uniform(0.1, 0.9)
                    else:
                        # No synergy relationship - explicit assignment for clarity
    ...
    def evolve_parameters(self, frame_count: int) -> None:
        """
        Evolve interaction parameters, give-take relationships, and synergy factors periodically.

        This method implements dynamic relationships between cellular types by mutating
        various interaction parameters at fixed intervals. These evolutionary changes
        create emergent behaviors and complex adaptive patterns over time.

        The evolution process affects three key areas:
        1. Interaction rule mutations (potential strength, gravity factor, max distance)
        2. Energy transfer efficiency adjustments across the system
        3. Synergy relationship strength modifications between cellular types

        Parameters
        ----------
        frame_count : int
            Current simulation frame count used to determine when evolution occurs.
            Evolution happens when frame_count is divisible by evolution_interval.
        """
        # Only evolve parameters at configured interval boundaries
        if frame_count % self.config.evolution_interval == 0:
            # Evolve interaction rules for each type pair
            for _, _, params in self.rules:
                # 10% chance to slightly mutate potential_strength (±5%)
                if random.random() < 0.1:
                    params["potential_strength"] *= random.uniform(0.95, 1.05)

                # 5% chance to slightly mutate gravity_factor if it exists (±5%)
                if random.random() < 0.05 and "gravity_factor" in params:
                    params["gravity_factor"] *= random.uniform(0.95, 1.05)

                # 5% chance to slightly mutate max_dist (±5%)
                if random.random() < 0.05:
                    # Ensure max_dist remains above minimum threshold of 10 units
                    params["max_dist"] = max(
                        10.0, params["max_dist"] * random.uniform(0.95, 1.05)
                    )

            # Evolve global energy transfer factor with 10% probability
            if random.random() < 0.1:
                # Adjust factor within ±5% while ensuring it doesn't exceed 1.0
                self.config.energy_transfer_factor = min(
                    1.0,  # Maximum energy transfer cap
                    self.config.energy_transfer_factor * random.uniform(0.95, 1.05),
                )

            # Evolve synergy matrix relationships with 5% probability per cell
...
class CellularTypeManager:
    ...
    def __init__(
        ...
        self.cellular_types: List[CellularTypeData] = (
            []
        )  # List to hold all cellular type data
        self.mass_based_type_indices = (
            mass_based_type_indices  # Indices of mass-based types
        )
    ...
    def remove_dead_in_all_types(self) -> None:
        ...
        for ct in self.cellular_types:
            ct.remove_dead(
                self.config
            )  # Remove dead components from each type with access to config
    ...
    def reproduce(self) -> None:
        ...
        for ct in self.cellular_types:
            ...
            if ct.mass_based and ct.mass is not None:
                ...
                if mutation_mask.any():
                    offspring_mass[mutation_mask] *= np.random.uniform(
                        0.95, 1.05, size=mutation_mask.sum()
                    )
                ...
                try:
                    ct.add_component(
                        ...
                        * offspring_traits["speed_factor"][i],
                        ...
                        mass_val=(
                            offspring_mass[i] if offspring_mass is not None else None
                        ),
                        energy_efficiency_val=offspring_efficiency[i],
                        speed_factor_val=offspring_traits["speed_factor"][i],
                        interaction_strength_val=offspring_traits[
                            "interaction_strength"
                        ][i],
...
def apply_interaction(
    a_x: float, a_y: float, b_x: float, b_y: float, params: Dict[str, Any]
) -> Tuple[float, float]:
    """
...
def give_take_interaction(
    ...
    """
    Transfer energy and optionally mass from receiver to giver based on configured factors.

    This function implements the predator-prey energy transfer mechanism, where the
    "receiver" loses energy (and potentially mass) which is then gained by the "giver".
    All operations maintain dimensional consistency across input arrays.

    Parameters
    ----------
    giver_energy : np.ndarray
        Energy levels of the particles receiving the energy transfer
    receiver_energy : np.ndarray
        Energy levels of the particles giving up energy
    giver_mass : Optional[np.ndarray]
        Mass values of receiving particles if mass-based; None otherwise
    receiver_mass : Optional[np.ndarray]
        Mass values of giving particles if mass-based; None otherwise
    config : SimulationConfig
        Configuration parameters controlling transfer amounts

    Returns
    -------
    Tuple[np.ndarray, np.ndarray, Optional[np.ndarray], Optional[np.ndarray]]
        Updated values for (giver_energy, receiver_energy, giver_mass, receiver_mass)
        with same shapes as inputs
    """
    # Calculate energy transfer amounts based on receiver's current energy levels
...
def apply_synergy(
    energyA: float, energyB: float, synergy_factor: float
) -> Tuple[float, float]:
    """
    ...
    newA = (energyA * (1.0 - synergy_factor)) + (
        avg_energy * synergy_factor
    )  # Update energy of A
    newB = (energyB * (1.0 - synergy_factor)) + (
        avg_energy * synergy_factor
    )  # Update energy of B
...
class Renderer:
    ...
    def render(self, stats: Dict[str, Any]) -> None:
        ...
        self.particle_surface.fill(
            (0, 0, 0, 0)
        )  # Clear particle surface for next frame
...
class CellularAutomata:
    ...
    def __init__(self, config: SimulationConfig):
        ...
        )  # Create optimized fullscreen window
        pygame.display.set_caption(
            "Emergent Cellular Automata Simulation"
        )  # Set window title
    ...
    def main_loop(self) -> None:
        ...
        while self.run_flag:
            ...
            self.display_fps(
                self.screen, current_fps
            )  # Pass the screen surface and current FPS
